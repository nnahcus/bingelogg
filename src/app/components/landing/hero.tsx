...
export default async function Hero() {
  ...
  let topAnimes: Anime[] = [];
  let topMangas: Manga[] = [];
  let topGames: IGDBgame[] = [];
  try {
    // ดึงมาจาก fetchtmdbapi/jikanapi + endpoint
    const [tmdbmovieData,tmdbtvData, jikananimeData, jikanmangadata, igdbgameData] = await Promise.all([
    ...
    topGames = igdbgameData ?? [];
  } catch (error) {
    // จับ error
    console.error("Error fetching popular movies:", error);
  }
  return (
    <section className= {`w-full text-xl md:text-2xl px-6 sm:px-8 md:px-12 lg:px-24 xl:px-36 pt-36  pb-6 md:pb-8 flex min-h-[70vh] items-center justify-center bg-linear-to-b from-[#242424] via-[#282829] to-[#39393B] ${interFont.className}`}>
      <div className="items-center">
        <div className="items-center text-center">
          <h1 className="text-4xl lg:text-5xl font-semibold leading-10 tracking-tight text-[#E5E5E5] dark:text-zinc-50 mb-8">
            Minimal tracking. Meaningful memories.
          </h1>
          <p className="text-lg md:text-2xl font-light leading-8 text-zinc-400  mb-8">
            A calm, minimal space to track what you've watched, read, played, and listened to.
          </p>
        </div>
        <button className="flex flex-col gap-4 text-base font-medium sm:flex-row items-center justify-center mx-auto pb-10">
          <a className="uppercase flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#C42138] px-5 text-white transition-colors hover:bg-[#871234] md:w-[158px]"
        ...
        <div className="">
          ...
          <div className="flex gap-4 overflow-x-auto justify-center">
            ...
            {/* TMDB API TV SHOW*/}
            {popularTVShows.length > 0 ? (popularTVShows.slice(0, 1).map((TV) => (
            ...
            {/* IGDB Game */}
            {topGames.length > 0 ? (topGames.slice(0, 1).map((game) => (
                <img key={game.id} src={game.cover ? fetchIGDBImage(game.cover.image_id, "t_1080p") : "/hero-fallback.jpg"}
                  alt={game.name} className="h-48 w-auto rounded-lg object-cover"/>))
            ) : (<p className="text-zinc-500">No popular game available</p>)}
            {/* OpenLibrary API Book */}
            <img src="https://covers.openlibrary.org/b/isbn/9780771038518-L.jpg" alt="The let them theory" className="h-48 w-auto rounded-lg object-cover" />
          </div>
        </div>       
      </div>
    </section>
